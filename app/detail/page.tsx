...
export default function Detail() {
  ...
  const updateItem = async (base64Image: string) => {
    try {
      const response = await fetch(`https://assignment-todolist-api.vercel.app/api/mandoo/items/${detailData?.id}`, {
        method: "PATCH",
        headers: {
  ...
  return (
    <div className="flex justify-center w-screen h-screen">
      <div className="w-full xl:w-[1200px] bg-white h-fit xl:h-full px-[100px]">
        <div className="w-full my-2 h-[60px] flex items-center">
          <input
            className={`w-full h-[60px] rounded-full border-2 text-center ${isCompleted ? 'bg-[#DDD6FE] border-black' : 'bg-white border-black'}`}
            value={nameValue}
            onChange={(e) => setNameValue(e.target.value)}
          />
          <button type="button" className="m-[15px] absolute" onClick={() => setIsCompleted(!isCompleted)}>
            <Image src={isCompleted ? '/ic/chFrame.svg' : '/ic/chDefault.svg'} alt="toggle" width={30} height={30} />
          </button>
        </div>
        ...
        <div className="flex flex-row justify-center xl:justify-end">
          <button
            type="submit"
            className={`w-[168px] h-[56px] m-2 flex flex-row justify-center items-center ${isCompleted ? 'bg-[#BEF264]' : 'bg-[#E2E8F0]'} border-2 border-black rounded-full text-center`}
            onClick={modClick}
          >
